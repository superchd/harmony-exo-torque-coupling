...
def angle_to_canvas_coords(sabd_deg, ef_deg):
    ...
    x = int((sabd_deg) /60 * WINDOW_WIDTH) #this changed window width t0 -60 t0 +60
    y = int((1 - ef_deg / 120) * WINDOW_HEIGHT) #caps window height at 120
    return x, y

# Define target angles
targets_def = [
    {"sabd": 0, "ef": 120},   # sets coordinates for targets
    {"sabd": 60, "ef": 0}     # sets coordinates for targets
]
...
cursor_x, cursor_y = angle_to_canvas_coords(-60 , 0)
...
def receive_udp_data():
    try:
        ...
        shoulder_abduction_rad = values[2 * 2]  # index 4
        elbow_flexion_rad = values[5 * 2]       # index 10

        # Convert to degrees
        sabd_deg = -math.degrees(shoulder_abduction_rad) #to get correct values, they had to be negative
        ef_deg = -math.degrees(elbow_flexion_rad) #to get correct values, they had to be negative
